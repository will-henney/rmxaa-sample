...
\documentclass[11pt,article,twocolumn]{RMxAC_rho-class/RMxAConecolumn_rho}
...
%\centering{\uppercase Example Abstract for the RMAA Conference Series}
...
\affil[2]{Universidad Nacional Autónoma de México, Instituto de Radioastronomía y Astrofísica} %Antigua Carretera a Pátzcuaro 8701, Ex-Hda. San José de la Huerta, 58089, Morelia, Michoacán, México}
...
\end{document}
    ...
    \subsection{Rhoenvs}
        ...
        This template has its own environment package \textit{rhoenvs.sty} designed to improve the presentation of information within documents. Among these custom environments are \textit{rhoenv}, \textit{info} and \textit{note}.

        All environments have the same style,with text to be highlighted in between: \CS{begin}\verb|{rhoenv}| ... \CS{end}\verb|{rhoenv}|; \CS{begin}\verb|{info}| ... \CS{end}\verb|{info}|;  and \CS{begin}\verb|{note} ... \CS{end}\verb|{note}|. 

        Rhoenv is the only environment that can be customized in terms of title. On the other hand, \textit{info} and \textit{note} adapt their title to Spanish automatically when the language package is defined.
    
        There are two environments that have a predefined title. These can be included by the command \verb|\begin{note}| and \verb|\begin{info}|. 
        
%   \newpage 	
    	An example using the rho environment is shown below.
    
        \begin{rhoenv}[frametitle=Environment with custom title]
            Example of the \textit{rhoenv} included in rhoenvs \LaTeX\ package, which may include relevant information or notes about your work. 
        \end{rhoenv}
